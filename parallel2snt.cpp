//changed by zs from plain2snt.cpp in giza++
...
int main(int argc,char**argv)
{
  ...
  if((filenames.size()%2)==1||filenames.size()==0 )
  {
    ...
    cerr << endl;
    cerr << "NOTE: \n";
    cerr << "      This is different from the plain2snt in giza++.\n";
    cerr << "      The language in txt1 is same with the one in txt2.\n";

    exit(1);
  }

  string line1,line2,word;
  map<string,int> v_freq;
  map<string,int> id;
  vector<string> iid(2);
  ...
  //substitute the filename ending with .txt or .tok without suffix
    ...
    while(getline(i1,line1) && getline(i2,line2) )
	  {
	    ...
	    {
	      osnt1 << w << "\n";
	      for(unsigned int j=0;j<t1.size();++j)osnt1 << id[t1[j]] << ' ';
	      osnt1 << '\n';
	      for(unsigned int j=0;j<t2.size();++j)osnt1 << id[t2[j]] << ' ';
	      osnt1 << '\n';

	      osnt2 << w << "\n";
	      for(unsigned int j=0;j<t2.size();++j)osnt2 << id[t2[j]] << ' ';
	      osnt2 << '\n';
	      for(unsigned int j=0;j<t1.size();++j)osnt2 << id[t1[j]] << ' ';
	      osnt2 << '\n';
	    }
	    else
  ...
  for(unsigned int i=2;i<iid.size();++i)
    ovocab1 << i << ' ' << iid[i] << ' ' << v_freq[iid[i]] << '\n';
  for(unsigned int i=2;i<iid.size();++i)
    ovocab2 << i << ' ' << iid[i] << ' ' << v_freq[iid[i]] << '\n';
